...
for (cond in conds) {
  for (plotBand_cort  in freqBands) {
    netData_sbjs = readMat(paste0(pathname, "antc_unmaskedConnStruct_netVals_old_", cond, "_early_epoch_PPT_aveTime_sbjs.mat"))
    print(paste0(pathname, "antc_unmaskedConnStruct_netVals_young_", cond, "_early_epoch_PPT_aveTime_sbjs.mat"))
    ...
    netData_ave = readMat(paste0(pathname, "antc_unmaskedConnStruct_netVals_old_", cond, "_early_epoch_PPT_aveTime.mat"))
    ...
    png(paste(pathname,cond,"_",plotBand_cort,"_antc_unmaskedConnStruct_brainEMG_old.png",sep=""),bg="transparent",width = 4160, height = 4160)
    ...
    circos.trackPlotRegion(ylim = c(0, 1), track.height = 0.05,
                           ...
                           panel.fun = function(x, y) {
                            ...
                            #              minor.ticks=1, labels.away.percentage = 0.15, lwd=2,labels.niceFacing=FALSE)
                             circos.axis(labels.cex=6, direction = "outside", major.at=c(0,2,4,6),#seq(from=0,to=floor(2),by=1),       # make adjustments if necessary
